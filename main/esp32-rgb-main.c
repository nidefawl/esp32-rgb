...
led_strip_handle_t configure_led(int n) {
  ...
  led_strip_config_t strip_config = {
    .strip_gpio_num = gpio,            // The GPIO that connected to the
...
#if HAS_GRBW
      ESP_ERROR_CHECK(led_strip_set_pixel_rgbw(led_strips[stripIndex], ledIndex, color[0], color[1], color[2], color[3]));
#else
      ESP_ERROR_CHECK(led_strip_set_pixel(led_strips[stripIndex], ledIndex, color[0], color[1], color[2]));
#endif
    }
...
void send_packet(int sock, struct sockaddr_storage *dest_addr, struct packet_hdr_t *header, void *message, int message_len) {
  char tx_buffer[128];
  memcpy(tx_buffer, header, sizeof(struct packet_hdr_t));
  memcpy(tx_buffer + sizeof(struct packet_hdr_t), message, message_len);
  int err = sendto(sock, tx_buffer, sizeof(struct packet_hdr_t) + message_len, 0, (struct sockaddr *)dest_addr, sizeof(struct sockaddr_storage));
  if (err < 0) {
    ESP_LOGE(TAG, "Error occurred during sending: errno %d", errno);
  }
}

void handle_packet(char *rx_buffer, int len, int sock, struct sockaddr_storage* source_addr) {
  ...
  if (hdr->packetType == PKT_TYPE_HEARTBEAT) {
    ...
    // ESP_LOGI(TAG, "Received config packet with cfgId %lu and value %lu\n", pkt->message.cfgId, pkt->message.value);
    ...
    switch (cfgId) {
      ...
      case CFG_ID_STRIPES_ENABLE:
        ...
        break;
      default:
        // ESP_LOGE(TAG, "Invalid config ID %u\n", pkt->message.cfgId);
        break;
    }
  } else if (hdr->packetType == PKT_TYPE_LED_FRAME) {
    struct packet_led_frame_t *pkt = (struct packet_led_frame_t *) rx_buffer;
    // ESP_LOGI(TAG, "PKT_TYPE_LED_FRAME %lu %lu\n", pkt->message.frameSize, pkt->message.frameOffset);
    if (pkt->message.frameOffset + pkt->message.frameSize > LED_STRIP_NUM_STRIPS * LED_STRIP_LED_NUMBERS) {
    ...
    int expectedSize = sizeof(struct packet_hdr_t) + sizeof(struct led_frame_message_t) + pkt->message.frameSize * 4;
    if (len != expectedSize) {
...
static void udp_server_task(void *pvParameters)
{
    ...
    while (1) {
        ...
        timeout.tv_sec = 0;  // infinite timeout
        timeout.tv_usec = 0;
        ...
        while (1) {
            ...
            // Data received
                ...
                // }
                handle_packet(rx_buffer, len, sock, &source_addr);
            }
        }
