...
class Lamp:
    ...
    def gen_frame(self, sock, frame_id):
        ...
        led_frame_data = process_pixels_rainbow_circle(
...
def process_pixels_rainbow_circle(time_since, frame_id, state, dimensions):
    ...
    for strip in range(LED_NUM_STRIPS):
        for led in range(LED_STRIP_NUM_LEDS_PER_STRIP):
            ...
            hue = dst * 360 * (0.5 + 0.5 * f + 5.5 * f * f2)
            speed = 64.0
...
def main(args=None):
    ...
    server = {"socket": sock, "port": port, "lamps": []}
    for lamp_ip in args.ips:
        ...
        lamp = Lamp(
            ...
            lamp_ip,
            port,
            socket.inet_pton(sock.family, lamp_ip),
        )
        ...
        if not lamp.load_config(sock):
            print("Error: could not load config for IP", lamp_ip)
            continue
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ips", type=str, nargs="+", help="List of IP addresses of the LED controller"
    )
    parser.add_argument(
        "--port",
        type=int,
